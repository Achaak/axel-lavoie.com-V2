function PictureTree() {
    ...
    function setAlbumDOM() {
        ...
        for (var i = 0; i < pictureTree.folders.length; i++) {
            ...
            // Prepare the picture
            var _folderDOM = $("<a href='" + folder + "/" + _folder.folder + "' class='photography-ctner'><div class='photo' style='background-image: url(" + (  _folder.files[0] ? (_folder.path.replace(/\\/g, "/") + "/" + _folder.files[0].name) : "/ressources/images/folder_open.png" ) + ")'></div><div class='infos'></div></a>");
